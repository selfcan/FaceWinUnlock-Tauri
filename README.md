...
| v0.3.6 | 2026-08-07 | Bug修复、系统增强 | 新增强制打开软件磁贴选项<br />修复睡眠休眠前、合盖进行面容解锁的bug<br />修复启用动态反馈时不能隐藏磁贴的问题<br />修复2项可能导致死锁的隐患<br />优化初始化时的锁屏界面提示<br />优化面容识别的动态反馈 (#123) <br />调整系统托盘图标<br />**需要重新初始化** |
| v0.3.7 | 2026-02-25 | 功能添加、BUG修复 | 新增快捷键解锁方式<br />添加多域账户的支持（不懂的别填）<br />左侧菜单添加其他首选项入口<br />修复清除缓存时无法打开缓存目录的bug<br />优化首选项界面的内存占用（小幅度）<br />**需要重新初始化** |
...
* [x] 日志清除功能
* [x] 解决睡眠、休眠前、笔记本合盖进行面容解锁的问题
* [x] 无面容时添加超时关闭功能
* [ ] 登录密码找回密码的功能
* [ ] 简化软件缓存清除功能
* [x] 延迟时间支持重试
* [x] 新的面容识别调用模式
* [x] 面容解锁分级支持（开机、锁屏、UAC、用户层）
* [ ] 活体检测优化（仍无法与2.2相比，待优化）
* [x] 一键卸载脚本（由claude生成）
* [ ] 检查更新功能（联网的功能暂时不考虑）
* [x] 识别时的动态反馈（26.02.19优化完成）
